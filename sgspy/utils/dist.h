...
calculateDist(
	...
	int width,
	int nBuckets,
	std::unordered_map<std::string, std::pair<std::vector<double>, std::vector<int>>>& retval,
	int nThreads)
{
	...
	std::string name = band.p_band->GetDescription();
	retval.emplace("population_" + name, {dbuckets, totalCounts});

	if (sampled.size() != 0) {
		std::vector<T> sampleCounts = sampleDistribution<T>(band, sampled, tbuckets, nBuckets);
		retval.emplace("sample_" + name, {dbuckets, sampleCounts});
	}
}
	...
	int nThreads)
{
	...
	std::pair<std::vector<double>, std::vector<int64_t>> retval;
