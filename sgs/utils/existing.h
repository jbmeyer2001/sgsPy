 ...
 * as parameters. 
 *
 * The points are stored as single values within an unordered map. This
 * map can then be checked to see if it contains particular index values.
 */
struct Existing {
	bool used;
	std::unordered_map<int64_t> samples;
	int64_t width;
	 ...
	 * is then stored in an unordered map.
	 *
	 ...
	 * @param int64_t width
	 */
	Existing(GDALVectorWrapper *p_vect, double *GT, int64_t width) {
		if (!p_vect) {
			this->used = false;
			return;
		}
