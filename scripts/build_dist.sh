...
auditwheel repair dist/*.whl -w dist --plat manylinux_2_39_x86_64 --exclude libmkl_core.so.2 --exclude libmkl_tbb_thread.so.2 --exclude libmkl_intel_ilp64.so.2 --exclude libonedal_parameters.so.3 --exclude libonedal_core.so.3 --exclude libonedal_thread.so.3 --exclude libonedal.so.3
rm dist/*linux_x86_64
